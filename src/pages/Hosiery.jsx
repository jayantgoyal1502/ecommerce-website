  ...
  useEffect(() => {
    fetch("http://localhost:5050/api/products")
      ...
      .then((data) => {
        ...
        const hosiery = data.filter((item) => {
          const t = item.type?.toLowerCase() || "";
          return t.includes("hosiery") || t.includes("socks") || t.includes("stockings") || t.includes("leggings");
        });
        setProducts(hosiery);
        setCategories(["All", ...Array.from(new Set(hosiery.map((p) => p.type)))]);
      });
  }, [msg]);
